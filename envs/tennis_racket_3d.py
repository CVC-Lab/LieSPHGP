...
class tennis_racket_3d(gym.Env):
    ...
    def __init__(
        ...
        # [FRICTION] friction_coeff: Union[float, Tuple] = 0.0,
        # [FRICTION] varying_friction: bool = False,
        # ── Misc ──────────────────────────────────────────────────────────
        ori_rep: str = "rotmat",
        ...
        # [FRICTION] self.friction_coeff = np.broadcast_to(
        # [FRICTION]     np.asarray(friction_coeff, dtype=np.float64), (3,)).copy()
        # [FRICTION] self.varying_friction = bool(varying_friction)
    ...
    # [FRICTION] def _variable_friction(self, omega: np.ndarray) -> np.ndarray:
    # [FRICTION]     if not self.varying_friction:
    # [FRICTION]         return self.friction_coeff
    # [FRICTION]     speed_term = np.tanh(np.linalg.norm(omega))
    # [FRICTION]     return self.friction_coeff * (1.0 + 0.5 * speed_term)
    ...
    def _compute_omega_rates(
        ...
        # [FRICTION] fric = self._variable_friction(omega)
        # [FRICTION] tau  = tau - fric * omega
