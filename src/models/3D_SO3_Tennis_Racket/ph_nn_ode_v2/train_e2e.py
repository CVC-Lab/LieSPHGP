"""End-to-end Stage A training: M_net is learned, not fixed.

Copied from train.py and extended to fix the Hamiltonian identifiability
degeneracy that arises when M_net is trained jointly with V_net.

Problem: without constraints, the optimizer can fit trajectories equally well
with (M_wrong, V_compensating) as with (M_true, V=0).  The degeneracy
manifests as M_loss diverging while test_geo_loss converges — M_net drifts
away from ground truth while V_net absorbs the residual.

Fixes applied here vs. train.py:
  1. --pretrain_M_lr defaults to 0.1  (was 1e-3 — too small for O(100) targets)
  2. --pretrain_M_steps defaults to 1000  (was 200)
  3. --lambda_V_zero: penalises mean(V_net(q)²) directly.  For a free rigid
     body V=0 is the correct physics; suppressing V forces M_net to account
     for all the kinetic energy and breaks the (M,V) trade-off.

Stage A end-to-end pass criteria (same as train.py):
  1. eval_M_loss  < 1e-4
  2. test_geo_loss < 0.01 rad²
  3. eval_V_loss small
  4. det(R̂) ≈ 1 (guaranteed by architecture)
"""
...
DEFAULT_SAVE_DIR = os.path.join(THIS_FILE_DIR, 'data', 'run_tr3d_e2e_fp32')
...
def get_args():
    ...
    # Defaults are higher than train.py: targets are O(100) so lr=0.1 and 1000 steps
    # are needed to converge before joint training begins.
    parser.add_argument('--pretrain_M_steps',       type=int,   default=1000)
    parser.add_argument('--pretrain_M_lr',          type=float, default=0.1)
    parser.add_argument('--pretrain_M_print_every', type=int,   default=100)
    ...
    # V-suppression: penalises mean(V_net(q)²) to break the (M,V) Hamiltonian degeneracy.
    # For a torque-free free body, V=0 is exact physics; this forces M_net to carry all
    # kinetic energy and makes it uniquely identifiable from trajectory data.
    parser.add_argument('--lambda_V_zero', type=float, default=1.0,
                        help='weight for V=0 suppression loss mean(V(q)²); 0 disables')
...
def build_run_name(args):
    ...
    parts = [
        ...
        f"lV0{_fmt_num(args.lambda_V_zero)}",
...
def train(args):
    ...
    stats = {
        ...
        'train_power_loss': [], 'train_V_zero_loss': [],
    ...
    for step in range(args.total_steps + 1):
        ...
        if args.lambda_V_zero > 0.0:
            q_flat = train_x_cat.detach().reshape(-1, 15)[:, :9]
            V_vals = _inner(model).V_net(q_flat).squeeze(-1)
            L_V_zero = (V_vals ** 2).mean()
        else:
            L_V_zero = torch.zeros((), device=device, dtype=float_type)
        ...
        loss_buffer.append(torch.stack([
            ...
            L_power.detach(), L_V_zero.detach(), L_V.detach(), L_B.detach(), L_D.detach()
        ]))
        ...
        if step % args.eval_every == 0:
            ...
            if loss_buffer:
                drained = torch.stack(loss_buffer, dim=0).cpu().numpy()  # (N, 8)
                ...
                stats['train_V_zero_loss'].extend(drained[:, 4].tolist())
                stats['train_V_cons_loss'].extend(drained[:, 5].tolist())
                stats['train_B_cons_loss'].extend(drained[:, 6].tolist())
                stats['train_D_cons_loss'].extend(drained[:, 7].tolist())
            ...
            train_V_zero = stats['train_V_zero_loss'][-1]
            ...
            print(f"  train: total={train_total:.4e}  "
                  ...
                  f"power={train_pow:.4e}  V0={train_V_zero:.4e}")
