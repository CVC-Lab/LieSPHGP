"""fp32 training for the tennis-racket free-rigid-body SO(3) Hamiltonian NODE.

Copied from 3D_SO3_Windy_Pendulum/ph_nn_ode_v2/train.py and adapted.

Differences from the pendulum version:
  - Loads the tennis-racket dataset directly from --data_path (a .pkl file).
  - Selects one geometry config via --config_idx (Stage A: single-config training).
  - FixedInertia replaces FixedInverseMass: returns diag(1/I1,1/I2,1/I3).
  - pretrain_M_net target is diag(1/I1,1/I2,1/I3) read from the dataset.
  - subnet diagnostics use subnet_physics_mse_tennis (V_tgt=0, Dw_tgt=0).
  - Pendulum-specific args removed: friction_coeff, wind, external_force, random_u.
  - network.py and loss_utils.py are imported from the shared pendulum ph_nn_ode_v2
    directory (identical architecture, no copy needed).

Stage A assumptions (torque-free, no friction):
  - disturbance_torque_std = 0  → Dw_tgt = 0  in diagnostics
  - V_net should converge to a near-constant (V_tgt = 0 for free body)
  - M_net should converge to diag(1/I1, 1/I2, 1/I3)
  - g_net should converge to I₃

Benchmarks to declare Stage A successful:
  1. eval_M_loss  < 1e-4  by end of training
  2. test_geo_loss < 0.01 rad² on the windowed eval
  3. eval_V_loss is small relative to eval_M_loss
  4. det(R̂) ≈ 1 throughout (guaranteed by architecture, verify numerically)
"""
...
DEFAULT_SAVE_DIR = os.path.join(THIS_FILE_DIR, 'data', 'run_tr3d_fp32')
