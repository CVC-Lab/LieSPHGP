...
Five scenarios:
  ...
  D. Hold e₁  with stochastic wind (disturbance_torque_std=0.05 N·m)
       — e₁ is stable; K_p_min=0 (no saddle), only ZOH bound K_p_max=2·I₁/T
  E. Hold e₃  with stochastic wind (disturbance_torque_std=0.05 N·m)
       — e₃ is stable; same ZOH bound applies
...
def scenario_D_e1_wind(args, I1, I2, I3, wind_std):
    """Hold e₁ with stochastic wind. K_p scan.

    e₁ is the stable (smallest-I) axis.  The free-body eigenvalue is imaginary
    (stable oscillation), so K_p_min = 0 in the absence of wind.  With wind the
    steady-state offset is |d|/K_p, so K_p must be large enough to keep that
    offset below the convergence threshold — and small enough to stay below the
    ZOH discrete-stability bound K_p_max = 2·I₁/T.
    """
    print(f"\n{'─'*60}")
    print(f"Scenario D: Hold e₁ with wind")
    print(f"  Target ω* = ({args.omega_star:.4f}, 0, 0) rad/s")
    print(f"  IC: ω* + N(0, {args.hold_perturb:.2f}²)")
    print(f"  Wind: disturbance_torque_std = {wind_std:.3f} N·m  (constant per episode)")
    kp_max = kp_max_zoh(I1, 0.05)
    lr, li = free_body_eigenvalue(I1, I2, I3, args.omega_star, target_axis=0)
    lam_str = f"±{li:.3f}i (stable osc)" if li > 1e-10 else f"{lr:.3f} (unstable)"
    print(f"  Free-body λ = {lam_str};  K_p_min=0  K_p_max={kp_max:.4f}")
    print(f"{'─'*60}")
...
def scenario_E_e3_wind(args, I1, I2, I3, wind_std):
    """Hold e₃ with stochastic wind. K_p scan.

    e₃ is the stable (largest-I) axis.  Same analysis as e₁+wind:
    K_p_min = 0 from stability, K_p_max = 2·I₁/T from ZOH,
    wind adds steady-state offset |d|/K_p.
    """
    print(f"\n{'─'*60}")
    print(f"Scenario E: Hold e₃ with wind")
    print(f"  Target ω* = (0, 0, {args.omega_star:.4f}) rad/s")
    print(f"  IC: ω* + N(0, {args.hold_perturb:.2f}²)")
    print(f"  Wind: disturbance_torque_std = {wind_std:.3f} N·m  (constant per episode)")
    kp_max = kp_max_zoh(I1, 0.05)
    lr, li = free_body_eigenvalue(I1, I2, I3, args.omega_star, target_axis=2)
    lam_str = f"±{li:.3f}i (stable osc)" if li > 1e-10 else f"{lr:.3f} (unstable)"
    print(f"  Free-body λ = {lam_str};  K_p_min=0  K_p_max={kp_max:.4f}")
    print(f"{'─'*60}")
...
def main():
    ...
    print("=" * 62)
    print("Stage E v2 — e₁/e₂/e₃ hold, no-wind + wind robustness")
    print("=" * 62)
    ...
    resD = scenario_D_e1_wind(args, I1, I2, I3, args.wind_std)
    resE = scenario_E_e3_wind(args, I1, I2, I3, args.wind_std)
    ...
    def best_stable_kp(res):
        """Return (best_kp, result) — highest convergence among stable Kps."""
        best = max(
            (k for k in res if res[k]['late_max'].mean() < 0.5),
            key=lambda k: res[k]['pct_converged'],
            default=None,
        )
        return best, (res[best] if best is not None else res[min(res)])

    C_best_kp, C_r_best = best_stable_kp(resC)
    C_pass = C_r_best['pct_converged'] >= 95

    D_best_kp, D_r_best = best_stable_kp(resD)
    D_pass = D_r_best['pct_converged'] >= 95

    E_best_kp, E_r_best = best_stable_kp(resE)
    E_pass = E_r_best['pct_converged'] >= 95

    kp_max = kp_max_zoh(I1, env_clean.dt)
    print(f"\n{'='*62}")
    print(f"Stage E summary  (Kp=0.10 unless noted; ZOH bound={kp_max:.3f})")
    print(f"  A — e₁ redirect (no wind):     {'PASS ✓' if A_pass else 'FAIL ✗'}")
    ...
    print(f"  D — e₁ hold (wind {args.wind_std:.2f} N·m):   "
          f"{'PASS ✓' if D_pass else 'FAIL ✗'}"
          + (f"  [best Kp={D_best_kp:.3f}]" if D_best_kp else ""))
    print(f"  E — e₃ hold (wind {args.wind_std:.2f} N·m):   "
          f"{'PASS ✓' if E_pass else 'FAIL ✗'}"
          + (f"  [best Kp={E_best_kp:.3f}]" if E_best_kp else ""))
    print(f"{'='*62}")
