"""Subnet streamplots on a regular (φ_i, p_i) grid.

Same setup as make_vectorfield_subnetwork_comparision.py but uses
matplotlib `streamplot` instead of `quiver`. The streamlines integrate
the per-subnet 2D vector field (φ̇_i, ṗ_subnet_i) over the (φ_i, p_i)
plane and visualise the flow induced by that subnet alone.
...
At every grid point (φ_i, p_i):
    full_eulers = mean_eulers (along the predicted trajectory) with
                  [i] replaced by grid φ_i.
    full_p      = mean_p with [i] replaced by grid p_i.
    R           = ZYX_intrinsic(full_eulers).
    ω           = (model M⁻¹)(R) · full_p.
    φ̇_i        = i-th component of the analytic ZYX Euler-rate map.
    ṗ_subnet_i  = the row's PHS subnet contribution to ṗ_i:
        M⁻¹ : (p × ω)_i
        V   : (−Σⱼ rⱼ × ∂_{qⱼ}V)_i        (potential-only)
        D   : −(D · ω)_i
        B   : (B · u)_i

Streamlines are drawn from this 2D field. Line *color* encodes the
streamline speed (‖(φ̇_i, ṗ_subnet_i)‖ in plot units). Line *width*
also scales with speed for legibility.

Predicted trajectories (used only for slice-anchor mean and grid range —
NOT plotted) come from the same rollouts as the vector-field/phase-plot
files: GP_SDE Lie-Heun, NN_ODE RK4 via torchdiffeq, GT env step.

β scale-invariance correction is applied per (model, checkpoint) so the
GP_SDE/NN_ODE outputs are on the GT scale: M⁻¹·β, V/β, D/β, B/β; p ←
p_raw/β. ω and φ̇ are β-invariant.
"""
...
DEFAULT_OUT_PDF = os.path.join(
    THIS_FILE_DIR, "streamplot_subnetwork_comparision.pdf")
...
def _build_grid_states(eulers_traj, p_traj, axis_i, n_grid):
    ...
    Returns: eulers_grid (N², 3), p_grid (N², 3),
             phi_axis (N,), p_axis (N,),
             Phi_mesh (N, N), P_mesh (N, N).
    Mesh uses indexing='xy' so streamplot can be fed the 1D axes directly
    and U/V matrices of shape (len(p_axis), len(phi_axis))."""
    ...
    return eulers_grid, p_grid, phi_axis, p_axis, Phi_mesh, P_mesh
...
def _process_model(name, traj_12, n_grid, u_const,
    ...
    if name == "GT":
        beta = 1.0
        p_traj = I_PERP * omegas_traj
    else:
        beta = _estimate_beta(M_inv_traj)
        p_traj = np.linalg.solve(
            M_inv_traj * beta, omegas_traj[..., None])[..., 0]

    cmap = {"GP_SDE": GP_CMAP, "NN_ODE": NN_CMAP, "GT": GT_CMAP}[name]

    per_axis = []
    for i in range(3):
        eulers_grid, p_grid, phi_axis, p_axis, Phi_mesh, P_mesh = (
            _build_grid_states(eulers_traj, p_traj, i, n_grid))
        ...
        N2 = qs_grid.shape[0]
        omegas_grid = (M_inv @ p_grid[..., None])[..., 0]
        phi_dot = omega_to_euler_rate_zyx(eulers_grid, omegas_grid)

        y_M = np.cross(p_grid, omegas_grid, axis=-1)
        y_V = neg_grav_V
        y_D = -(D @ omegas_grid[..., None])[..., 0]
    ...
    return dict(per_axis=per_axis, cmap=cmap), beta
...
def _stream(ax, phi_axis, p_axis, U, V, cmap, density=1.2):
    """Streamplot with line color/width encoding speed.

    matplotlib.streamplot needs strictly monotonically-increasing X, Y;
    np.linspace gives that. U, V must have shape (len(Y), len(X)) — we
    built the meshgrid with indexing='xy' so this is already the case.
    """
    speed = np.sqrt(U ** 2 + V ** 2)
    smax = float(speed.max()) if speed.size else 0.0
    if smax < 1e-12:
        # Degenerate field (e.g. GT M⁻¹ row: p × ω = 0). Skip — leave
        # the cell visibly empty rather than draw NaN streamlines.
        ax.text(0.5, 0.5, "field ≡ 0", ha="center", va="center",
                transform=ax.transAxes, color="gray", fontsize=9,
                style="italic")
        return
    lw = 0.5 + 2.0 * (speed / smax)
    ax.streamplot(phi_axis, p_axis, U, V,
                  color=speed, cmap=cmap, linewidth=lw,
                  density=density, arrowsize=0.9, arrowstyle="-|>")


def make_page(step, per_model_data, betas, density):
    ...
    for r, (sub, model) in enumerate(rows):
        ...
        for c in range(3):
            ax = axes[r, c]
            a = d["per_axis"][c]
            _stream(ax, a["phi_axis"], a["p_axis"],
                    a["phi_dot_i"], a[SUBNET_KEY[sub]],
                    cmap=cmap, density=density)
    ...
    fig.suptitle(
        f"Subnet streamplots on (φ_i, p_i) grid  —  step = {step}",
...
def main():
    ...
    ap.add_argument("--n_grid", type=int, default=40,
                    help="grid resolution per axis (n_grid × n_grid). "
                         "Streamplot interpolates, so denser is fine here "
                         "than in the quiver version.")
    ap.add_argument("--density", type=float, default=1.4,
                    help="streamplot density argument (line spacing)")
    args = ap.parse_args()
    ...
    gt_data, _ = _process_model("GT", gt_traj, args.n_grid, u_const)
    ...
    with PdfPages(args.out_pdf) as pdf:
        for step in args.steps:
            ...
            fig = make_page(step, data, betas, args.density)
