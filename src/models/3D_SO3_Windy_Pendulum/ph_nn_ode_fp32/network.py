...
class DissipativeSO3HamNODE(torch.nn.Module):
    def __init__(self, M_net=None, Dw_net=None, V_net=None, g_net=None,
        ...
        # Dw_net input is (q, p) ∈ ℝ¹² so the network can capture
        # state-dependent friction whose magnitude depends on |p|/|ω|
        # (e.g. the env's tanh(|ω|) term in `varying_friction`).
        if friction:
            self.Dw_net = Dw_net or PSD(self.rotmatdim + self.angveldim, 20,
                                        self.angveldim,
    ...
    def forward(self, t, x):
        with torch.enable_grad():
            ...
            if self.friction:
                # Dw_net takes (q, p) — the canonical pH momentum, not ω.
                Dw_q = self.Dw_net(torch.cat((q, p), dim=1))
